%% RPCA from continuous sampling with post-iteration integer rounding on S
% End-to-end script: builds a high-rate multitone, folds it, samples both at a
% lower rate, and recovers the unfolded samples via Hankel(Δy) + RPCA.
% IMPORTANT: Integer rounding of S is applied *after* IALM converges.
% After rounding, we enforce equality by Lbar := Mbar - Sbar_round.
...
alpha  = 2*lambda;      % *** correct scaling for Δy-based Hankel ***
...
%% -------- 4) RPCA on Hankel(Δ y) with POST-ITER INTEGER ROUNDING --------
% Work on first difference of the folded samples, build a Hankel matrix,
% solve PCP in the scaled domain (divide by alpha), *no rounding during IALM*,
% then after convergence: Sbar := round(Sbar), Lbar := Mbar - Sbar.
...
% Run PCP without integer projection inside the loop; round at the end
lam_pcp = 1/sqrt(max(size(Mbar)));
[HLbar_pre, HSbar_pre, out_pre] = rpca_pcp_noint(Mbar, lam_pcp, opts);

% ---- Post-iteration integer rounding and constraint enforcement ----
HSbar = round(HSbar_pre);            % nearest-integer projection (AFTER convergence)
HLbar = Mbar - HSbar;                % enforce equality Mbar = Lbar + Sbar
...
% Residuals before/after rounding (in scaled domain)
res_pre  = norm(Mbar - HLbar_pre - HSbar_pre, 'fro') / max(norm(Mbar,'fro'),eps);
res_post = norm(Mbar - HLbar - HSbar,       'fro') / max(norm(Mbar,'fro'),eps);

fprintf(['[BASE] PCP (post-round): L=%d | %dx%d | RelErr=%.3g | SNR=%.2f dB\n' ...
         '       Residual pre-round=%.2e  post-round=%.2e\n'], ...
        L, size(H,1), size(H,2), rel_err, snr_db, res_pre, res_post);

% Plot recovery at sample instants
figure('Name','RPCA recovery at FS\_SAMPLE (post-round)','NumberTitle','off','Position',[60 760 1100 320]);
plot(t_samp, x_samp, 'LineWidth', 1.25); hold on;
plot(t_samp, x_hat, '--', 'LineWidth', 1.25); grid on;
xlabel('Time (s)'); ylabel('Amplitude');
title(sprintf('Recovery (post-iter round) at f_s=%.0f Hz | L=%d | SNR=%.2f dB', FS_SAMPLE, L, snr_db));
legend('x (true sampled)','\\hat{x} (recovered)','Location','best');

disp('Done. Integer rounding applied after IALM, with Lbar reset to satisfy equality.');
...
% ---------- Standard PCP (NO integer projection inside the loop) ----------
function [Lbar,Sbar,out] = rpca_pcp_noint(Mbar, lambda_pcp, opts)
% Solve:  min ||Lbar||_* + lambda*||Sbar||_1  s.t. Mbar = Lbar + Sbar
% Using IALM/SVT with soft-thresholding on S; NO integer rounding inside.
    ...
    if verbose
        fprintf('RPCA-PCP (no-int): lambda=%.3g, tol=%.1e\n', lambda_pcp, tol);
    ...
    while (relres > tol) && (k < max_iter)
        ...
        % === Sbar update: soft-threshold (NO rounding here) ===
        T = Mbar - Lbar + (1/mu)*Y;
        Sbar = sign(T) .* max(abs(T) - lambda_pcp/mu, 0);
        ...
        obj = sum(s_shr) + lambda_pcp * sum(abs(Sbar(:)));
